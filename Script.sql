...
-- Proc insere usuario
CREATE
OR REPLACE PROCEDURE insere_usuario(CPF char(11), nome Nome, email varchar(50)) LANGUAGE SQL AS $ $
INSERT INTO
    Usuario
VALUES
    (CPF, nome, email);
...
CREATE
OR REPLACE PROCEDURE MembrosInterno(
    ...
    WHERE
        U.cpf = CPF2
) THEN CALL insere_usuario(CPF2, nome, email);
...
CREATE
OR REPLACE PROCEDURE MembrosExterno(CPF2 char(11), nome Nome, email varchar(50)) AS $ $ BEGIN IF NOT EXISTS(
    SELECT
        *
    FROM
        usuario U
...
--insere pesquisador
CREATE
OR REPLACE PROCEDURE insere_pesquisador(
    titulaçao varchar(30),
    lattes varchar(256),
    CPF2 char(11)
) AS $ $ BEGIN if NOT exists(
    select
...
INSERT INTO
    Pesquisador
VALUES
    (titulaçao, lattes, CPF2);

else RAISE NOTICE 'Não é possivel adicionar um pesquisadoor que já é bolsista';
...
OR REPLACE procedure insere_bolsista(
    ...
    where
        b.cpf = cpf2
        and b.orientado [array_length(b.orientado,1)].fim < orientado_p.inicio
) THEN
UPDATE
    Bolsista
set
    orientado [array_length(orientado,1)] = orientado_p
where
    cpf = cpf2;
...
INSERT INTO
    Bolsista
VALUES
    (array [orientado_p], CPF2);

ELSE RAISE NOTICE 'Não é possivel adicionar mais de um orientador por periodo de tempo';

END IF;

END IF;

else RAISE NOTICE 'Não é possivel adicionar um bolsista que já é pesquisador';

end if;

ELSE RAISE NOTICE 'Orientador não existe.';
...
--insere camera
CREATE
OR REPLACE PROCEDURE insere_camera(
    marca varchar(100),
    lente varchar(100),
    modelo varchar(100),
    larguraPixels int,
...
--insere local
CREATE
OR REPLACE PROCEDURE insere_local(
    cnpj char(14),
    nome varchar(100),
    campanha varchar,
...
--insere PlanoDeVoo
CREATE
OR REPLACE PROCEDURE insere_planodevoo (
    cnpj char(14),
    altitude float,
    gsd float,
...
CREATE
OR REPLACE PROCEDURE insere_pontos_de_controle(
    points geometry(point, 4674),
    "Tag" varchar,
    "Long" float,
    "Lat" float,
    cnpj char(14),
...
CREATE
OR REPLACE PROCEDURE insere_orthomosaico(
    imagemOrtho bytea,
    arquivoGis bytea,
...
CREATE
OR REPLACE PROCEDURE insere_Imagem(
    imagem bytea,
    dataHora timestamp,
    CPF char(11),
    idVoo int,
...
CREATE
OR REPLACE PROCEDURE atualiza_descricao_Imagem(
    idImagemP int,
...
CREATE
OR REPLACE PROCEDURE atualiza_Orthomosaico_da_Imagem(idImagemP int, idOrthomosaicoP int) LANGUAGE SQL AS $ $
UPDATE
    Imagem
SET
    idOrthomosaico = idOrthomosaicoP
WHERE
    idImagem = idImagemP;
...
CREATE
OR REPLACE FUNCTION confere_ponto_cidade (id_ponto INT, id_cidade INT) RETURNS boolean AS $ $ DECLARE pertence boolean;

BEGIN
SELECT
    ST_Contains(m.geom, p.geom) INTO pertence
FROM
    "Municipios_MT_2021" m
...
CREATE
OR REPLACE FUNCTION encontra_ponto_por_cidade (id_cidade INT) RETURNS SETOF geometry AS $ $ DECLARE pontos geometry;
...
FROM
    ...
    JOIN pontos p ON ST_Contains(m.geom, p.geom)
WHERE
    m.id = id_cidade LOOP RETURN NEXT pontos;
...
CREATE
OR REPLACE FUNCTION encontra_cidade (id_ponto INT) RETURNS text AS $ $ DECLARE nome_cidade text;

BEGIN
SELECT
    m.nm_mun INTO nome_cidade
FROM
    "Municipios_MT_2021" m
    JOIN pontos p ON ST_Contains(m.geom, p.geom)
WHERE
