...
BEGIN
    IF EXISTS(SELECT * FROM pesquisador p WHERE orientado_p.orientador = p.cpf) THEN  -- precisa ter um pesquisador
        if NOT exists(select * from pesquisador p WHERE cpf2=p.cpf) THEN -- o bolsista nao pode ser um pesquisador
            if EXISTS(select * from Bolsista b where b.cpf=cpf2 and b.orientado[array_length(b.orientado,1)].fim < orientado_p.inicio) THEN
                UPDATE Bolsista set orientado[array_length(orientado,1)] = orientado_p where cpf = cpf2; -- atualiza datas
            else
